...
import { checkPin, requestPin } from "@/state/userData/getUserData";
...
const ChangePin = ({ navigation }: any) => {
    ...
    function handleSubmit() {
        ...
        dispatch(checkPin(formData))
            .then(async (result) => {
                if (result.payload.success === true) {
                    try {
                        navigation.push("SetAuthScreen3");
                        setLoading2(false);
                    } catch (err) {
                        console.log(err);
                        setLoading2(false);
    ...
    function sendRequest() {
        setDisplay2("");
        const formdata: formdata7 = {
            email: data.email,
        };
        dispatch(requestPin(formdata))
            .then(async (result) => {
                console.log(result.payload, "result");
                console.log(result.payload.success, "success");
                if (result.payload.success === true) {
                    setDisplay2(languageText.text287);
                    setLoading2(false);
                }
                if (result.payload.status === 401) {
                    displayNotification();
