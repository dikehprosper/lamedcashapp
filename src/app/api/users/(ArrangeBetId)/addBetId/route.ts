  ...
  try {
    ...
    if (!user) {
      ...
      if (user.supplementaryBetId.length >= 2) {
        return NextResponse.json({ error: "maximum betId added" }, { status: 401});
      } 
      
    const hasDuplicateBetId = user.supplementaryBetId.some((id: any) => id === betId);
if (hasDuplicateBetId) {
    return NextResponse.json({ error: "betId already exists" }, { status: 402 });
}
