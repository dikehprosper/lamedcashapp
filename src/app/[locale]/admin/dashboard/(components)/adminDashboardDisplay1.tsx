...
export default function AdminDashboardDisplay1({data3, updatedTheme}: any) {
  ...
  const completedDeposits = data3?.transactionHistory // Flatten the array of objects into an array of transactionHistory arrays
    .filter(
  ...
  const failedDeposits = data3?.transactionHistory // Flatten the array of objects into an array of transactionHistory arrays
    .filter(
      (transaction: {status: string; registrationDateTime: string}) =>
        (state !== "All Time" &&
  ...
  const PendingDeposits = data3?.pendingTransactions // Flatten the array of objects into an array of transactionHistory arrays
    .filter(
      (transaction: {status: string}) => transaction.status === "Pending"
    );

  const sum = PendingDeposits?.reduce(
    (acc: any, obj: any) => acc + obj.totalAmount,
    0
  );
  const sum2 = completedDeposits?.reduce(
    (acc: number, obj: {totalAmount: number}) => acc + Number(obj.totalAmount),
    0
  );

  const sum3 = failedDeposits?.reduce(
    (acc: number, obj: {totalAmount: number}) => acc + Number(obj.totalAmount),
    0
  );
