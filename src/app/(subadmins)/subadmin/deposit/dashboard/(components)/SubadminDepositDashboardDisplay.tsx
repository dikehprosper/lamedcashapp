...
const [loading, setLoading] = useState(false)
  ...
  // Filter transactions based on the calculated start time
   const completedOrders = data?.transactionHistory.filter(
    (transaction: { status: string; registrationDateTime: string }) =>
      (state !== "All Time" && transaction.status === 'Successful' && new Date(transaction.registrationDateTime) >= new Date(calculateStartTime())) ||
  ...
  return (  
    <div className='subadmin_dashboard_container-display'>
      ...
      { data ?
      <div className='subadmin_dashboard_container-display-children animate-pop-in'>
        ...
        <h4>XOF 
          ...
          {formatNumberWithCommasAndDecimal(sum)}
            ...
            <div className='subadmin_dashboard_container-display-children-loading '>
      <div id='container_deposit_display'>
                  <div id='container_deposit_display_inner'></div>
                </div>
                </div>
                }
      ...
      <div id='container_deposit_display'>
                ...
                </div>: <> <h1 className="animate-pop-in">
          
          {completedOrders?.length}
          
          </h1>
        <h4 className="animate-pop-in">XOF 
          &nbsp;
