...
const TransactionTemplate = () => {
  ...
  const getUserDetails = async () => {
    try {
      ...
      const res = await axios.post("/api/getUserCashdesk", { id: idFromUrl });
      console.log(res)
...
<div className='transaction_template_container_header'>
      ...
      <div className='transaction_template_container' >
        ...
        >
          {data?.length < 1 ? (
            <div style={{width:"100%", height: "100%", paddingTop: "50px", display: "flex", justifyContent: "center"}}> <h2>No data to display</h2></div>
           
          ) : (
            data?.map((filteredData: any, index: any) => (
